...
let r1 = () => bass.low()
let r2 = () => drums.mid()
let movement = () => Math.sin(time * 0.22) * (0.03 + F1 * 0.08 + r1() * 0.04)
let pulse = () => 0.5 + Math.sin(time * 0.45) * (0.25 + r2() * 0.35)
...
let base = osc(3, 0.04, 0.4)
  ...
  .repeatX(() => 2 + F2 * 3)
  .repeatY(2)
  .modulate(noise(2, 0.18), () => 0.08 + pulse() * 0.18)
  .contrast(1.4)
...
let vertical = osc(7, 0.02, 0.5)
  .rotate(() => -movement())
  .repeatX(5)
  .modulate(noise(3, 0.12), () => 0.06 + r1() * 0.16)
  .contrast(1.8)

// TEXTURA ORGÁNICA ===========================================
let organic = noise(2, 0.2)
  .pixelate(() => 25 + F2 * 35, 100)
  .modulate(osc(2, 0.05, 0.3), () => 0.08 + r2() * 0.12)
  .contrast(1.5)
...
base
  ...
  .modulate(noise(1.5, 0.15), () => 0.05 + N12 * (0.06 + r2() * 0.14))
  .contrast(1.3)
  .out()
