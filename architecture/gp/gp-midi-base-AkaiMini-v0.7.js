...
// (ruido/salto en el reposo). Un fader listado ahí queda forzado
// a 0 mientras su valor crudo esté por debajo del umbral.
//
...
;(function () {
  ...
  if (window.GP.midi && window.GP.midi.version === '0.7-AkaiMini') {
    console.info('[GP MIDI] AkaiMini v0.7 ya está cargada')
    return
  }

  const midi = {}
  const STORAGE_KEY = 'GP_AkaiMini_Faders_v0.7'
  ...
  // Zona muerta por fader (0..1). Mientras el valor crudo esté por
  ...
  function aplicarDeadzone(nombre, valor) {
    const piso = FADER_DEADZONE[nombre] || 0
    return valor < piso ? 0 : valor
  }
  ...
  function registerListeners() {
    ...
    channel.onCC('*', ({ index, value }) => {
      if (index >= 48 && index <= 56) {
        ...
        const limpio = aplicarDeadzone(nombre, value)
  ...
  midi.state = state
  window.GP.midi = midi
  console.info('[GP MIDI] base AkaiMini v0.7 cargada')
})()
