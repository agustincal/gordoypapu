...
//
// v0.8: FADER_DEADZONE ahora además RESCALA. Antes (v0.7) solo
// forzaba a 0 por debajo del umbral, y el resto del recorrido
// quedaba tal cual — bien para una zona muerta chica (ruido de
// reposo), pero si el tramo poco confiable es grande (por ejemplo
// un fader que manda valores erráticos de 0 a 60/127), dejar el
// resto sin reescalar te comía casi la mitad de la resolución útil.
// Ahora: por debajo del umbral → 0. Por encima → se estira ese
// tramo (umbral..1) para que ocupe todo el 0..1 lógico de nuevo.
//
// v0.7: agrega FADER_DEADZONE — zona muerta por fader para
// controladoras donde el potenciómetro no descansa en 0 real
// (ruido/salto en el reposo, o un tramo entero poco confiable).
//
...
;(function () {
  ...
  if (window.GP.midi && window.GP.midi.version === '0.8-AkaiMini') {
    console.info('[GP MIDI] AkaiMini v0.8 ya está cargada')
    return
  }

  const midi = {}
  const STORAGE_KEY = 'GP_AkaiMini_Faders_v0.8'
  ...
  // Zona muerta + reescalado por fader (0..1, sobre el valor ya
  // normalizado que llega de hydra-midi).
  //   - por debajo del umbral: se fuerza a 0 (se ignora el ruido)
  //   - desde el umbral hasta 1: se reescala a 0..1 de nuevo, para
  //     no perder resolución en el tramo que sí funciona bien.
  ...
  const FADER_DEADZONE = {
    F7: 60 / 127   // ~0.47 — errático hasta acá en esta controladora
  }
  ...
  midi.state = state
  window.GP.midi = midi
  console.info('[GP MIDI] base AkaiMini v0.8 cargada')
})()
