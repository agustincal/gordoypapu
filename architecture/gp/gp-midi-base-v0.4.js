...
//
// Usa hydra-midi 0.4.3 como capa MIDI externa.
// ======================================================
;(function () {
  window.GP = window.GP || {}

  if (window.GP.midi && window.GP.midi.version === '0.4') {
    console.info('[GP MIDI] v0.4 ya está cargada')
    return
  }
  ...
  function registerListeners() {
    ...
    // hydra-midi ya entrega los CC normalizados entre 0 y 1.
  ...
  midi.start = async function ({ outputName } = {}) {
    ...
    try {
      // Versión fija para que todos los equipos usen la misma capa MIDI.
      ...
      if (!selected) throw new Error('GP MIDI: no se encontró una salida MIDI')
  ...
  midi.state = state
  window.GP.midi = midi
  console.info('[GP MIDI] base v0.4 cargada')
})()
