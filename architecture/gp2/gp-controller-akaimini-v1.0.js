// ======================================================
// G&P — GP CONTROLLER: AKAI MINI (APC Mini) — v1.0
//
// Esta es UNA implementación de la interfaz GP.midi.
// Para soportar otra controladora el día de mañana, escribí
// gp-controller-<nombre>.js que exponga ESTA MISMA interfaz:
//
//   GP.midi.start({ outputName? })   → conecta, arma listeners
//   GP.midi.faders(names[])          → activa faders por nombre
//   GP.midi.buttons(names[])         → activa botones por nombre
//   GP.midi.fader(name)              → 0..1
//   GP.midi.button(name)             → true/false
//   GP.midi.on(name, cb)             → callback al togglear botón
//   GP.midi.onFader(name, cb)        → callback al mover fader
//   GP.midi.show(bool)               → mostrar/ocultar monitor
//   GP.midi.reset()                  → limpia estado + LEDs
//   GP.midi.devices()                → { inputs[], outputs[] }
//   GP.midi.isReady()                → bool
//   globals F1..F8, FMASTER, N11..N88
//
// Mientras el nuevo archivo respete esta interfaz, ni el core
// (gp-core.js) ni los sketches necesitan cambiar una línea.
// ======================================================
;(function () {
  window.GP = window.GP || {}

  const VERSION = 'akaimini-1.0'
  if (window.GP.midi && window.GP.midi.version === VERSION) {
    console.info('[GP CONTROLLER] AkaiMini ya está cargado')
    return
  }
  ...
  midi.start = async function ({ outputName } = {}) {
    ...
    try {
      ...
      if (!selected) {
        ...
        console.info('[GP CONTROLLER] sin controladora: usando valores por defecto')
        return midi
      }
      ...
      state.output = null
      state.outputName = null
      console.info('[GP CONTROLLER] MIDI no disponible: usando valores por defecto')
      return midi
    }
  }
  ...
  midi.state = state
  window.GP.midi = midi
  console.info('[GP CONTROLLER] AkaiMini ' + VERSION + ' cargado')
})()
